...
describe("node request codecs", () => {
	it("returns parsing failures and enforces bounded defaults", async () => {
		const server = await createNodeAdapter(createBodyParsingImplementations(), {
			createHandlerOptions: { requestBody: { maxBytes: 2 } },
		}).start();
		try {
			const malformed = await fetch(`${server.origin}/body-parsing/json`, {
				method: "POST",
			...
			const oversized = await fetch(`${server.origin}/body-parsing/json`, {
				method: "POST",
				headers: { "content-type": "application/json" },
				body: "{} ",
			});
			...
			const chunked = await fetch(`${server.origin}/body-parsing/json`, {
				method: "POST",
				headers: { "content-type": "application/json" },
				...
				bodyCodecs: [
					{
						...
						deserialize: async (request) => {
							assert.equal(request.url, "/body-parsing/json");
	...
	it("propagates custom errors and rejects unsupported types before matching", async () => {
		...
		try {
			const rejected = await fetch(`${server.origin}/body-parsing/json`, {
				method: "POST",
			...
			const failed = await fetch(`${server.origin}/body-parsing/json`, {
				method: "POST",
				headers: { "content-type": "application/json" },
				body: "{}",
			});
