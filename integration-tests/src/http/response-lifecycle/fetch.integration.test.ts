...
runResponseMiddlewareHeadersSuite(
	createFetchAdapter(createResponseLifecycleImplementations(), {
...
const lifecycleContract = {
	contextMutation: route
		.get("/response-lifecycle/context-mutation")
		.response(200, z.object({ ok: z.literal(true) })),
	returnResponse: route
		.get("/response-lifecycle/return-response")
		.response(200, z.object({ ok: z.literal(true) })),
};
...
describe("fetch response lifecycle integration", () => {
	...
	it("treats returned Response objects as invalid route response bodies", async () => {
		const response = await fetch(
			`${server.origin}/response-lifecycle/return-response`,
		);
		...
		assert.deepEqual(await response.json(), {
			code: "INVALID_RESPONSE",
			path: "/response-lifecycle/return-response",
		});
	});
});
