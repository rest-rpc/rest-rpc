...
Clients serialize typed request input and parse declared server responses as the
contract's response output type. Response validation is normally enforced on the
server before data is sent; set `validateResponses: true` when you want the
client to additionally validate declared HTTP responses, stream chunks, and
