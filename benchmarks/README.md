...
The benchmark independently measures TypeScript checker cost for explicit and
shorthand contract-first declarations and server-first implementations across
the built-in type-only schema helper and the main supported validation
...
The scripts generate matching explicit and shorthand fixtures under
`generated/contract-only/`, `generated/server-first/`,
`generated/contract-shorthand/`, and `generated/server-first-shorthand/`, then
run `tsc --noEmit --extendedDiagnostics` for each schema library and route
count. Console and Markdown output contain a separate table for each style.
