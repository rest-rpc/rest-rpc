...
For schema records, headers, and JSON query parameters, the converted JSON Schema
does not always say whether the parameter itself is required. Use
`isSchemaRequired` to read that from your schema library metadata.
...
const document = createOpenApiDocument(api, {
	...
	isSchemaRequired: ({ jsonSchema }) => {
		const metadata = jsonSchema as
			{ openApi?: { required?: boolean } } | undefined;

		return metadata?.openApi?.required ?? true;
	},
});
```
