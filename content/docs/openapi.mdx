...
Use `.openAPI()` to describe one route. Its position among the request and
response setters does not affect the generated operation.
	...
	todos: {
		list: route
			...
			.openAPI({
				...
				security: [{ bearerAuth: [] }],
				responses: {
					200: { headers: { "x-request-id": z.string() } },
					401: { description: "Authentication is required." },
				},
			})
			.response(200, z.array(todoSchema))
			.response(401, z.object({ code: z.string() })),
	},
};
```

Typed headers declared on a route response override OpenAPI-only headers with
the same name in generated OpenAPI.
