...
encode form, multipart, and custom request bodies. JSON query values still use
the `request.jsonQuery()` helper.
...
Requests declared with `body(schema, contentType)` must always supply a
`contentType` option to a server-first client.
