...
For form, multipart, and JSON query values, wrap the value with the matching
`request.formBody()`, `request.multipartBody()`, or `request.jsonQuery()` helper
so the client knows how to encode it.
