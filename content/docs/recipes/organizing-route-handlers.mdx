...
const routes = router(api, {
	todos: {
		async get(request) {
			const todo = await store.get(request.params.id);
...
class TodoService implements RouteHandlers<typeof api.todos> {
	...
	async get(request: RouteRequest<typeof api.todos.get>) {
		const todo = await this.store.get(request.params.id);
	...
	async create(request: RouteRequest<typeof api.todos.create>) {
		const todo = await this.store.create({ title: request.body.title });
...
class TodoService implements RouteHandlers<typeof api.todos> {
	...
	get({ params: { id } }: RouteRequest<typeof api.todos.get>) {
...
class TodoService implements RouteHandlers<typeof api.todos> {
	get(request: RouteRequest<typeof api.todos.get>) {
		return this.store.get(request.params.id);
	}
}
```
...
class TodoService implements RouteHandlers<typeof api.todos> {
	...
	get(request: RouteRequest<typeof api.todos.get>) {
		return {
			...
			title: `${this.prefix}-${request.params.id}`,
		};
	}
}
```
