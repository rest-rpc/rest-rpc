	...
	todos: {
		...
		create: route
			...
			.handler(({ input, context }) => {
				return createTodo(input.title, context.signal);
			}),
	},
};
```
...
The handler receives validated procedure input under `input`, so input schemas
may describe any JSON value and an input field named `context` cannot conflict
with the handler context. Clients still pass the input value directly.
