...
| Input fits in one JSON value                                | You need path, query, form, file, or stream data        |
...
the successful JSON result. Input uses JSON by default and accepts the same
optional content-type argument as `.body()`. An output is required; input is
optional. Form, multipart, and custom input values follow the transport-shape
rules in [HTTP Requests](/docs/http-requests).
