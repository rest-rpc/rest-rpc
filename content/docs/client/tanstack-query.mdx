...
description: Create typed TanStack Query helpers and combine them with your Tanstack Query setup to call your routes over HTTP.
---
...
Routes expose typed query, mutation, pagination, stream and key helpers:
...
api.todos.events.streamedQueryOptions(...)
```

:::note
`streamedQueryOptions()` is only available for routes with one successful or inferred
`.streamResponse(status, schema)` or a plain `.streamOutput(schema)`.
:::

## Setup

Client configuration uses the same setup and options as the [Fetch Client](/docs/client/fetch-client),
