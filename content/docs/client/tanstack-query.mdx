...
`streamedQueryOptions()` is generated for plain output streams or response
streams with exactly one successful `.streamResponse()`. Use `queryOptions()`
for the raw async iterable. With `.streamResponse()`, `queryOptions()` also
retains `status` and `headers`; with `.streamOutput()`, it returns the iterable
directly.
