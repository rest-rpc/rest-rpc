...
`.streamResponse(status, schema)` response. The helper passes the route's async
iterable body to TanStack Query and stores the materialized stream data, not the
rest-rpc response envelope.
...
response and that response body must be declared with `.streamResponse()`. Use
