...
Routes with one successful `.streamResponse(status, schema)` response also
expose `streamedQueryOptions()`.
...
Use `streamedQueryOptions()` for explicit routes that declare one successful
`.streamResponse(status, schema)` response or procedures declared with
`.streamOutput(schema)`. The helper passes the async iterable to TanStack Query
and stores the materialized stream data. For an explicit route it discards the
rest-rpc response envelope; procedure clients already return the iterable
directly.
...
`streamedQueryOptions()` is only generated for procedure streams or explicit
stream routes where rest-rpc can hide the response envelope honestly. An
explicit route must have exactly one successful response and that response must
be declared with `.streamResponse()`. Use `queryOptions()` for the raw async
iterable. On an explicit route, `queryOptions()` also retains `status` and
`headers`; on a procedure it returns the iterable directly.
...
This differs from calling an HTTP route directly, which exposes declared non-2xx
responses as values.
