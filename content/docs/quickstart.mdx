---
title: Quickstart
description: Define one contract, implement it on the server, and call it from a client.
---

This guide builds one `todos.create` route from a shared contract.

<Steps>
  <Step title="Install packages">
    Add the core package, a schema library, and the adapter for the server
    framework you want to use.
  </Step>
  <Step title="Define the contract">
    Declare the HTTP method, path, request body, and response body once in a file
    that both server and client code can import.
  </Step>
  <Step title="Implement the server route">
    Use the same route tree to write a typed handler for Express, Fastify, Hono,
    NestJS, Next.js, or a fetch-native runtime.
  </Step>
  <Step title="Call it from the client">
    Create a typed fetch client or TanStack Query helpers from the same contract.
  </Step>
</Steps>

## Install Packages

This guide uses Zod and Express by default. The same contract works with the
other server tabs below.

<Tabs>
<Tab title="pnpm">

```bash
pnpm add @rest-rpc/core @rest-rpc/express express zod
```

</Tab>
<Tab title="npm">

```bash
npm install @rest-rpc/core @rest-rpc/express express zod
```

</Tab>
<Tab title="yarn">

```bash
yarn add @rest-rpc/core @rest-rpc/express express zod
```

</Tab>
<Tab title="bun">

```bash
bun add @rest-rpc/core @rest-rpc/express express zod
```

</Tab>
</Tabs>

If you use another server tab, replace `@rest-rpc/express express` with the
adapter and framework package you need:

- Fastify: `@rest-rpc/fastify fastify`
- Hono: `@rest-rpc/hono hono`
- NestJS: `@rest-rpc/nest @nestjs/common @nestjs/core @nestjs/platform-express reflect-metadata rxjs`
- Next.js: `@rest-rpc/next`
- Web `Request`/`Response`: `@rest-rpc/web`

For the TanStack Query client example, also install:

```bash
pnpm add @rest-rpc/tanstack-query @tanstack/react-query
```

## Define A Contract

Create a shared contract file. Put it somewhere both your server and client can
import.

<Tabs>
<Tab title="Zod">

```ts contract.ts
import { router } from "@rest-rpc/core";
import { z } from "zod";

const todoSchema = z.object({
	id: z.string(),
	title: z.string(),
	completed: z.boolean(),
});

export const api = router({
	todos: {
		create: {
			method: "POST",
			path: "/todos",
			body: {
				title: z.string().min(1),
			},
			response: todoSchema,
		},
	},
});
```

</Tab>
<Tab title="Valibot">

```ts contract.ts
import { router } from "@rest-rpc/core";
import * as v from "valibot";

const todoSchema = v.object({
	id: v.string(),
	title: v.string(),
	completed: v.boolean(),
});

export const api = router({
	todos: {
		create: {
			method: "POST",
			path: "/todos",
			body: {
				title: v.pipe(v.string(), v.minLength(1)),
			},
			response: todoSchema,
		},
	},
});
```

</Tab>
<Tab title="ArkType">

```ts contract.ts
import { router } from "@rest-rpc/core";
import { type } from "arktype";

const todoSchema = type({
	id: "string",
	title: "string",
	completed: "boolean",
});

export const api = router({
	todos: {
		create: {
			method: "POST",
			path: "/todos",
			body: {
				title: type("string>0"),
			},
			response: todoSchema,
		},
	},
});
```

</Tab>

<Tab title="None">

```ts contract.ts
import { router, type } from "@rest-rpc/core";

const todoSchema = {
	id: type<string>(),
	title: type<string>(),
	completed: type<boolean>(),
};

export const api = router({
	todos: {
		create: {
			method: "POST",
			path: "/todos",
			body: {
				title: type<string>(),
			},
			response: todoSchema,
		},
	},
});
```
</Tab>
</Tabs>

## Server implementation

Implement the contract on the server with the framework of your choice.

<Tabs param="server">
<Tab title="Express">

```ts twoslash
// ---cut-start---
// @filename: contract.ts
import { router } from "@rest-rpc/core";
import { z } from "zod";

const todoSchema = z.object({
	id: z.string(),
	title: z.string(),
	completed: z.boolean(),
});

export const api = router({
	todos: {
		create: {
			method: "POST",
			path: "/todos",
			body: {
				title: z.string().min(1),
			},
			response: todoSchema,
		},
	},
});

// @filename: server.ts
// ---cut-end---
import { registerRoutes, router } from "@rest-rpc/express";
import express from "express";
import { api } from "./contract";

const app = express();
app.use(express.json());

const todos = new Map<string, {
	id: string;
	title: string;
	completed: boolean;
}>();

const routes = router(api, {
	todos: {
		create({ title }) {
			const todo = {
				id: crypto.randomUUID(),
				title,
				completed: false,
			};

			todos.set(todo.id, todo);

			return todo;
		},
	},
});

registerRoutes(app, routes);

app.listen(3000);
```

</Tab>

<Tab title="Fastify">

```ts twoslash
// ---cut-start---
// @filename: contract.ts
import { router } from "@rest-rpc/core";
import { z } from "zod";

const todoSchema = z.object({
	id: z.string(),
	title: z.string(),
	completed: z.boolean(),
});

export const api = router({
	todos: {
		create: {
			method: "POST",
			path: "/todos",
			body: {
				title: z.string().min(1),
			},
			response: todoSchema,
		},
	},
});

// @filename: server.ts
// ---cut-end---
import { registerRoutes, router } from "@rest-rpc/fastify";
import Fastify from "fastify";
import { api } from "./contract";

const app = Fastify();

const todos = new Map<string, {
	id: string;
	title: string;
	completed: boolean;
}>();

const routes = router(api, {
	todos: {
		create({ title }) {
			const todo = {
				id: crypto.randomUUID(),
				title,
				completed: false,
			};

			todos.set(todo.id, todo);

			return todo;
		},
	},
});

registerRoutes(app, routes);

await app.listen({ port: 3000 });
```

</Tab>

<Tab title="Hono">

```ts twoslash
// ---cut-start---
// @filename: contract.ts
import { router } from "@rest-rpc/core";
import { z } from "zod";

const todoSchema = z.object({
	id: z.string(),
	title: z.string(),
	completed: z.boolean(),
});

export const api = router({
	todos: {
		create: {
			method: "POST",
			path: "/todos",
			body: {
				title: z.string().min(1),
			},
			response: todoSchema,
		},
	},
});

// @filename: server.ts
// ---cut-end---
import { registerRoutes, router } from "@rest-rpc/hono";
import { Hono } from "hono";
import { api } from "./contract";

const app = new Hono();

const todos = new Map<string, {
	id: string;
	title: string;
	completed: boolean;
}>();

const routes = router(api, {
	todos: {
		create({ title }) {
			const todo = {
				id: crypto.randomUUID(),
				title,
				completed: false,
			};

			todos.set(todo.id, todo);

			return todo;
		},
	},
});

registerRoutes(app, routes);

export default app;
```

</Tab>

<Tab title="NestJS">

```ts twoslash
// @experimentalDecorators: true
// ---cut-start---
// @filename: contract.ts
import { router } from "@rest-rpc/core";
import { z } from "zod";

const todoSchema = z.object({
	id: z.string(),
	title: z.string(),
	completed: z.boolean(),
});

export const api = router({
	todos: {
		create: {
			method: "POST",
			path: "/todos",
			body: {
				title: z.string().min(1),
			},
			response: todoSchema,
		},
	},
});

// @filename: server.ts
// ---cut-end---
import "reflect-metadata";
import { Controller, Injectable, Module } from "@nestjs/common";
import { NestFactory } from "@nestjs/core";
import {
	initNest,
	RestRpcModule,
	Router,
	type RouteHandlers,
	type RouteRequest,
} from "@rest-rpc/nest";
import { api } from "./contract";

const nest = initNest();

@Injectable()
class TodoService implements RouteHandlers<typeof api.todos> {
	private readonly todos = new Map<string, {
		id: string;
		title: string;
		completed: boolean;
	}>();

	create({ title }: RouteRequest<typeof api.todos.create>) {
		const todo = {
			id: crypto.randomUUID(),
			title,
			completed: false,
		};

		this.todos.set(todo.id, todo);

		return todo;
	}
}

@Controller()
class TodoController {
	constructor(private readonly todos: TodoService) {}

	@Router(api.todos)
	todosRouter() {
		return nest.router(api.todos, this.todos);
	}
}

@Module({
	imports: [RestRpcModule.forRoot()],
	controllers: [TodoController],
	providers: [TodoService],
})
class AppModule {}

const app = await NestFactory.create(AppModule);
await app.listen(3000);
```

</Tab>

<Tab title="Next.js">

```ts twoslash
// ---cut-start---
// @filename: contract.ts
import { router } from "@rest-rpc/core";
import { z } from "zod";

const todoSchema = z.object({
	id: z.string(),
	title: z.string(),
	completed: z.boolean(),
});

export const api = router({
	todos: {
		create: {
			method: "POST",
			path: "/todos",
			body: {
				title: z.string().min(1),
			},
			response: todoSchema,
		},
	},
});

// @filename: route.ts
// ---cut-end---
import { createRouteHandler, router } from "@rest-rpc/next";
import { api } from "./contract";

const todos = new Map<string, {
	id: string;
	title: string;
	completed: boolean;
}>();

const routes = router(api).handlers({
	todos: {
		create({ title }) {
			const todo = {
				id: crypto.randomUUID(),
				title,
				completed: false,
			};

			todos.set(todo.id, todo);

			return todo;
		},
	},
});

export const { POST } = createRouteHandler(routes);
```

</Tab>
</Tabs>

## Client implementation

Use the same contract to create a typed client.

<Tabs>
<Tab title="Fetch Client">

```ts twoslash
// ---cut-start---
// @filename: contract.ts
import { router } from "@rest-rpc/core";
import { z } from "zod";

const todoSchema = z.object({
	id: z.string(),
	title: z.string(),
	completed: z.boolean(),
});

export const api = router({
	todos: {
		create: {
			method: "POST",
			path: "/todos",
			body: {
				title: z.string().min(1),
			},
			response: todoSchema,
		},
	},
});

// @filename: client.ts
// ---cut-end---
import { initClient } from "@rest-rpc/core";
import { api } from "./contract";

const client = initClient(api, {
	baseUrl: "http://localhost:3000",
});

const todo = await client.todos.create.fetch({
	title: "Ship v1",
});
```

</Tab>
<Tab title="TanStack Query">

```tsx twoslash
// ---cut-start---
// @filename: contract.ts
import { router } from "@rest-rpc/core";
import { z } from "zod";

const todoSchema = z.object({
	id: z.string(),
	title: z.string(),
	completed: z.boolean(),
});

export const api = router({
	todos: {
		create: {
			method: "POST",
			path: "/todos",
			body: {
				title: z.string().min(1),
			},
			response: todoSchema,
		},
	},
});

// @filename: client.tsx
// ---cut-end---
import { initTanstackQuery } from "@rest-rpc/tanstack-query";
import { useMutation } from "@tanstack/react-query";
import { api } from "./contract";

const tq = initTanstackQuery(api, {
	baseUrl: "http://localhost:3000",
});

const createTodo = useMutation(
	tq.todos.create.mutationOptions({
		onSuccess(response) {
			console.log(response.body.id);
		},
	}),
);

createTodo.mutate({
	title: "Ship v1",
});
```

</Tab>
</Tabs>

## Next Steps

- [Contract Declaration](/docs/contract/declaration/) explains the route declaration shape.
- [Shorthand vs Explicit](/docs/shorthand-vs-explicit/) shows the concise defaults
  shown here next to the explicit alternatives.
- [Schemas](/docs/contract/schemas/) explains validation and type-only schemas.
- [Server adapters](/docs/server/express/) cover framework-specific setup.
- [Fetch Client](/docs/client/fetch-client/) and [TanStack Query](/docs/client/tanstack-query/)
  cover client behavior in more detail.
