...
- Path params in the URL must have matching `pathParams` schema keys.
- `pathParams` schema keys must correspond to actual path params.
...
Query values, path params, and headers are serialized into HTTP fields, so their
