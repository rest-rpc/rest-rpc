...
```ts
import { noBody, route } from "@rest-rpc/core";

export const api = {
	todos: {
		remove: route.delete("/todos/:id").response(204, noBody()),
	},
};
```
...
export const api = {
	todos: {
		get: route.get("/todos/:id").response(200, {
			body: todoSchema,
			headers: {
				etag: z.string(),
...
export const api = {
	reports: {
		csv: route.get("/reports.csv").response(
			200,
			customBody({
...
export const api = {
	images: {
		download: route.get("/images/:id").response(
			200,
			customBody({
...
Use `mode: "sse"` for a GET route that returns server-sent events. The route declares one event schema with `response`, and the server yields `sseEvent()` values.
...
export const api = {
	notifications: {
		live: route.sse("/notifications/live").response(200, notificationSchema),
	},
};
```
...
```ts
import { route, stream } from "@rest-rpc/core";
...
export const api = {
	ai: {
		tokens: route
			...
			.response(200, stream(tokenSchema)),
	},
};
```
...
export const api = {
	files: {
		download: route.get("/files/:id").response(
			200,
			stream(
				customBody({
