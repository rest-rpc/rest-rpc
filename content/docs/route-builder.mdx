...
Use the route inference helpers when the request or response type needs to be
reused elsewhere in your application.
...
const create = route
	.input(z.object({ title: z.string() }))
	.handler(({ input }) => ({ id: "todo_1" as const, title: input.title }));
...
type CreateTodoRequest = InferClientRequest<typeof create>;

type UpdateTodoRequest = InferClientRequest<typeof update>;
```

### Infer Client Response
...
const list = route.handler(() => [{ id: "todo_1", title: "Write docs" }]);
...
type ListTodosResponse = InferClientResponse<typeof list>;

type GetFirstTodoResponse = InferClientResponse<typeof getFirst>;
```

### Infer Server Request

```ts twoslash
import type { InferServerRequest } from "@rest-rpc/express";
import { route } from "@rest-rpc/express";
import z from "zod";
...
type CreateTodoRequest = InferServerRequest<typeof create>;

type UpdateTodoRequest = InferServerRequest<typeof update>;
```

### Infer Server Response

```ts twoslash
import type { InferServerResponse } from "@rest-rpc/express";
import { route } from "@rest-rpc/express";
import z from "zod";
...
const getFirst = route
	...
	.handler(({ params }) => {
		...
		if (random < 0.5) {
			return { status: 200, body: { id: params.id, title: "Write docs" } };
		}
		return { status: 404, body: { code: "TODO_NOT_FOUND" as const } };
	});

type ListTodosResponse = InferServerResponse<typeof list>;

type GetFirstTodoResponse = InferServerResponse<typeof getFirst>;
```
